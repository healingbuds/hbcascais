...
// Three environments: production, staging, railway
...
function getEnvironment(requestedEnv?: string): EnvConfig {
  // If explicit environment requested, use it (production, staging, or railway)
...
/**
 * Resolve the clientCartId from a client profile.
 * The Dr. Green API requires clientCartId (the cart's own UUID) for cart operations,
 * which is different from clientId. This fetches the client profile and extracts it.
 * Falls back to listing all clients if the direct GET returns 401.
 */
async function resolveClientCartId(clientId: string, envConfig?: EnvConfig): Promise<string> {
  logInfo("Resolving clientCartId from client profile", { clientId: clientId.slice(0, 8) + '***' });
  
  // Try direct client fetch first (uses signBody pattern)
  try {
    const directResponse = await drGreenRequestBody(`/dapp/clients/${clientId}`, "GET", { clientId }, false, envConfig);
    if (directResponse.ok) {
      const directData = await directResponse.json();
      // Check multiple nesting levels for clientCart
      const clientData = directData?.data?.data || directData?.data || directData;
      const clientCart = clientData?.clientCart;
      if (Array.isArray(clientCart) && clientCart.length > 0 && clientCart[0]?.id) {
        const cartId = clientCart[0].id;
        logInfo("Resolved clientCartId via direct fetch", { 
          clientCartId: cartId.slice(0, 8) + '***',
          clientId: clientId.slice(0, 8) + '***',
        });
        return cartId;
      }
      logWarn("Direct client fetch OK but no clientCart found", { 
        hasData: !!clientData,
        keys: clientData ? Object.keys(clientData).join(',') : 'none',
      });
    } else {
      logWarn("Direct client fetch failed", { status: directResponse.status });
    }
  } catch (err) {
    logWarn("Direct client fetch exception", { error: String(err).slice(0, 100) });
  }
  
  // Fallback: list all clients and find by ID
  logInfo("Falling back to client list to resolve clientCartId");
  try {
    const listResponse = await drGreenRequestGet("/dapp/clients", { take: 200, page: 1, orderBy: "desc" }, false, envConfig);
    if (listResponse.ok) {
      const listData = await listResponse.json();
      // Response shape varies: data.clients, data.data, data.items, or data as array
      const rawData = listData?.data;
      const clients = Array.isArray(rawData) ? rawData 
        : rawData?.clients || rawData?.data || rawData?.items || [];
      ...
      if (matchedClient) {
        const clientCart = matchedClient.clientCart;
        if (Array.isArray(clientCart) && clientCart.length > 0 && clientCart[0]?.id) {
          const cartId = clientCart[0].id;
          logInfo("Resolved clientCartId via list fallback", { 
            clientCartId: cartId.slice(0, 8) + '***',
          });
          return cartId;
        }
        logWarn("Client found in list but no clientCart", { 
          keys: Object.keys(matchedClient).join(','),
        });
      } else {
        logWarn("Client not found in list", { 
          totalClients: Array.isArray(clients) ? clients.length : 0,
        });
      }
    }
  } catch (err) {
    logWarn("Client list fallback exception", { error: String(err).slice(0, 100) });
  }
  
  // If all resolution attempts fail, throw descriptive error
  throw new Error(
    `Could not resolve clientCartId for client ${clientId.slice(0, 8)}***. ` +
    `The Dr. Green API requires a clientCartId (cart UUID) from the client profile, ` +
    `but neither direct fetch nor list fallback returned a valid clientCart.`
  );
}
...
/**
 * Normalize list responses from the Dr. Green API.
 * The API returns arrays in varying nesting: data.data, data.clients,
 * data.items, data.orders, or data as a direct array.
 * This always returns { items: [...], meta: { total, page, take } }.
 */
function normalizeListResponse(rawData: any): { items: any[]; meta: { total: number; page: number; take: number } } {
  if (!rawData) return { items: [], meta: { total: 0, page: 1, take: 10 } };
  ...
  if (Array.isArray(d)) {
    ...
    items = d.orders;
  } else if (Array.isArray(d?.nfts)) {
    items = d.nfts;
  } else {
    items = [];
  }

  // Extract pagination meta from various locations
  const meta = d?.meta ?? d?.pagination ?? rawData?.meta ?? {};
  return {
    items,
    meta: {
      total: meta.total ?? meta.totalCount ?? items.length,
      page: meta.page ?? meta.currentPage ?? 1,
      take: meta.take ?? meta.perPage ?? meta.limit ?? items.length,
    },
  };
}
 ...
 * Unwraps double-nested structures like { data: { data: { id, ... } } }
 * into the inner object.
 */
function normalizeSingleResponse(rawData: any): any {
  if (!rawData) return null;
  const d = rawData?.data ?? rawData;
  // If there's another .data inside, unwrap it
...
/**
 * Normalize client-specific responses.
 * Unwraps nesting and converts shippings[] → shipping{} for consistency.
 */
function normalizeClientResponse(rawData: any): any {
  const client = normalizeSingleResponse(rawData);
  if (!client) return null;

  // Normalize shippings array to single shipping object
  if (Array.isArray(client.shippings) && client.shippings.length > 0 && !client.shipping) {
    client.shipping = client.shippings[0];
  }

  return client;
}

/** Map of actions to their normalizer type */
const ACTION_NORMALIZER_MAP: Record<string, 'list' | 'single' | 'client'> = {
  'dapp-clients': 'list',
  'dapp-orders': 'list',
  'dapp-clients-list': 'list',
  'get-client-orders': 'list',
  'get-orders': 'list',
  'dapp-nfts': 'list',
  'dapp-client-details': 'client',
  'get-client': 'client',
  ...
  try {
    ...
    switch (action) {
      ...
      case "get-my-details": {
        ...
        try {
          // Per API guide: GET /dapp/clients/{id} signs a JSON signBody (not sent in request)
          apiResponse = await drGreenRequestBody(`/dapp/clients/${clientId}`, "GET", { clientId });
      ...
      case "dapp-order-details": {
        ...
        // Per API guide: GET /dapp/orders/{id} signs a JSON signBody (not sent in request)
        response = await drGreenRequestBody(`/dapp/orders/${orderId}`, "GET", { orderId }, false, adminEnvConfig);
        break;
      }
      ...
      case "get-client": {
        ...
        // Per API guide: GET /dapp/clients/{id} signs a JSON signBody (not sent in request)
        const clientResponse = await drGreenRequestBody(`/dapp/clients/${body.clientId}`, "GET", { clientId: body.clientId }, false, adminEnvConfig);
      ...
      case "add-to-cart": {
        ...
        // - clientCartId (UUID) - the CART's own UUID, NOT the clientId
        // - items[].strainId (UUID) - the strain/product ID
        // - items[].quantity (number)
        // Resolve the actual clientCartId from the client profile
        const resolvedCartId = await resolveClientCartId(clientId, adminEnvConfig);
        
        const cartPayload = {
          clientCartId: resolvedCartId,
          items: [
            {
        ...
        logInfo("Adding to cart", { 
          clientCartId: resolvedCartId.slice(0, 8) + '***',
          clientId: clientId.slice(0, 8) + '***',
          strainId: cartData.strainId, 
          quantity: cartData.quantity,
        });
      ...
      case "create-order": {
        ...
        // ========== MOCK MODE CHECK ==========
        // Only allow mock mode in non-production environments (staging/railway)
        const mockEnvName = (envConfig?.name || 'production').toLowerCase();
        const isMockAllowed = mockEnvName === 'staging' || mockEnvName === 'railway';
        if (Deno.env.get('DRGREEN_MOCK_MODE') === 'true' && isMockAllowed) {
          logInfo(`[${requestId}] MOCK MODE (${mockEnvName}): Simulating successful order creation`);
          ...
          return new Response(JSON.stringify({
            success: true,
            orderId: mockOrderId,
            message: `[MOCK MODE - ${mockEnvName}] Order simulated successfully - no actual order was created`,
            mockMode: true,
            requestId,
            items: mockItems,
        ...
        // clientCartId is the CART's own UUID, NOT the clientId — resolve it first
        const resolvedOrderCartId = await resolveClientCartId(clientId, adminEnvConfig);
        ...
        const cartPayload = {
          clientCartId: resolvedOrderCartId,
          items: cartItems,
        };
        
        logInfo(`[${requestId}] Step 2: Adding items to cart`, { 
          itemCount: cartItems.length,
          clientCartId: resolvedOrderCartId.slice(0, 8) + '***',
          clientId: clientId.slice(0, 8) + '***',
        });
      ...
      case "get-order": {
        ...
        // Per API guide: GET /dapp/orders/{id} signs {"orderId": "..."} as signBody
        const signBody = { orderId: body.orderId };
        response = await drGreenRequestBody(`/dapp/orders/${body.orderId}`, "GET", signBody);
        break;
      }
      ...
      case "get-orders": {
        ...
        // Per API guide: GET /dapp/client/{clientId}/orders signs {"clientId": "..."} as signBody
        response = await drGreenRequestBody(
          `/dapp/client/${body.clientId}/orders`,
          "GET",
    ...
    // Apply response normalization based on action type
    const normalizerType = ACTION_NORMALIZER_MAP[action];
    let normalizedData = data;
    if (normalizerType && response.ok) {
      try {
        switch (normalizerType) {
          case 'list':
            normalizedData = normalizeListResponse(data);
            break;
          case 'single':
            normalizedData = normalizeSingleResponse(data);
            break;
          case 'client':
            normalizedData = normalizeClientResponse(data);
            break;
        }
        logInfo(`Normalized response for action '${action}' using '${normalizerType}' normalizer`);
      } catch (normErr) {
        logWarn(`Normalization failed for action '${action}', returning raw`, { error: String(normErr).slice(0, 100) });
        normalizedData = data;
      }
    }
