...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Starting client sync from Dr Green API...");
    ...
    while (hasMore) {
      const response = await drGreenGet("/dapp/clients", { orderBy: 'desc', take, page });
      if (!response.ok) {
      ...
      hasMore = allClients.length < total && clients.length > 0;
      page++;
      if (page > 20) break;
    }

    console.log(`Fetched ${allClients.length} clients from Dr Green API`);
    ...
    for (const client of allClients) {
      try {
        ...
        if (upsertError) {
          errors.push(`${client.email}: ${upsertError.message}`);
        } else {
          synced++;
        }
      } catch (err) {
    ...
    return new Response(
      JSON.stringify({ success: true, synced, created, updated, errors, total: allClients.length }),
