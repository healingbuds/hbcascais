...
function calculateItemsHash(items: OrderItem[]): string {
  return items.map(i => `${i.strain_id}:${i.quantity}`).sort().join('|');
}
...
export function useOrderTracking() {
  ...
  const syncFromDrGreen = useCallback(async () => {
    ...
    try {
      ...
      // Unwrap: API may return { items: [...], meta: {...} } (normalized proxy),
      // { success, data: [...] }, raw array, or nested data.data
      let ordersList: any[] = [];
      ...
      for (const live of ordersList) {
        ...
        const normalizedItems = (live.items || []).map((item: any) => ({
          strain_id: item.strainId || item.strain_id || '',
          strain_name: item.strainName || item.strain_name || item.name || 'Unknown',
