...
export function useAdminOrderSync() {
  ...
  const fetchAllOrders = useCallback(async (filters: OrderFilters = {}): Promise<{
    ...
    if (search) {
      query = query.or(`drgreen_order_id.ilike.%${search}%,customer_email.ilike.%${search}%,customer_name.ilike.%${search}%`);
    }
    ...
    const orders: LocalOrder[] = await Promise.all(
      (data || []).map(async (order: any) => {
        ...
        // Guard: skip lookup if user_id is null to prevent 400 errors
        if (!order.user_id) {
          return {
            ...order,
            items: order.items || [],
  ...
  const syncOrderMutation = useMutation({
    mutationFn: async (orderId: string): Promise<OrderSyncResult> => {
      ...
      // Fallback to client lookup only if order doesn't have stored context AND user_id is present
      if ((!clientId || !shippingAddress) && order.user_id) {
      ...
      // Also try client_id-based lookup if we still don't have what we need
      if ((!clientId || !shippingAddress) && order.client_id) {
        const { data: client } = await supabase
          .from("drgreen_clients")
          .select("drgreen_client_id, shipping_address")
          .eq("drgreen_client_id", order.client_id)
          .maybeSingle();
