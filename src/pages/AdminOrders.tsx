...
const AdminOrders = () => {
  ...
  // Initialize orders from the hook (only when not actively filtering)
  useEffect(() => {
    if (!isFiltering) {
      setOrders(initialOrders);
    }
  }, [initialOrders, isFiltering]);

  // Filter orders when tab or search changes (debounced)
  useEffect(() => {
    const timer = setTimeout(async () => {
      // Skip filtering if using default "all" tab with no search
  ...
  }, [activeTab, searchQuery, fetchOrders, initialOrders]);
