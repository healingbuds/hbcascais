...
const AdminDashboard = () => {
  ...
  // Auto-sync on admin login — data stays live
  useEffect(() => {
    loadDashboard();
    // Auto-sync clients and orders from Dr Green API on mount
    handleAutoSync();
  }, []);
  ...
  const handleAutoSync = async () => {
    ...
    try {
      ...
      const clientsSynced = clientsRes.data?.synced || 0;
      const ordersSynced = ordersRes.data?.synced || 0;

      setLastSyncAt(new Date());
      await Promise.all([fetchStats(false), fetchRecentActivity()]);

      console.log(`[Auto-sync] ${clientsSynced} clients, ${ordersSynced} orders synced`);
    } catch (error: any) {
  ...
  const handleSyncAll = async () => {
    ...
    try {
      ...
      toast({
        title: "Sync Complete",
        description: `${clientsSynced} clients and ${ordersSynced} orders synced from Dr. Green API.`,
      });
    } catch (error: any) {
      console.error('Sync all error:', error);
      toast({
        title: "Sync Failed",
        description: error.message || "Failed to sync data from API.",
        variant: "destructive",
      });
    } finally {
      setSyncingAll(false);
  ...
  return (
    ...
    >
      <div className="space-y-8">
        {/* Top Bar: Refresh + Sync */}
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <div className={`w-2 h-2 rounded-full ${syncingAll ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`} />
            <span>
              {syncingAll ? 'Syncing...' : lastSyncAt ? `Live • synced ${formatDistanceToNow(lastSyncAt, { addSuffix: true })}` : 'Live'}
            </span>
          </div>
